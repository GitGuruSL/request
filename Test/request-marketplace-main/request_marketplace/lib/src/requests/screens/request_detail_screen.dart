...
import 'respond_to_rental_request_screen.dart';
    ...
    return Container(
      ...
      child: SizedBox(
        ...
        child: FloatingActionButton.extended(
          heroTag: "request_detail_fab", // Unique hero tag
          onPressed: () async {
            if (widget.request.type == RequestType.ride && !_isDriver) {
              showDialog(
                context: context,
            ...
            final result = await Navigator.push(
              context,
              MaterialPageRoute(
                builder: (context) => widget.request.type == RequestType.service
                    ? RespondToServiceRequestScreen(request: widget.request)
                    : widget.request.type == RequestType.ride
                        ? RespondToRideRequestScreenNew(request: widget.request)
                    : widget.request.type == RequestType.delivery
                        ? RespondToDeliveryRequestScreen(request: widget.request)
                    : widget.request.type == RequestType.rental
                        ? RespondToRentalRequestScreen(request: widget.request)
                        : RespondToRequestScreen(request: widget.request),
              ),
            );
            if (result == true) {
