...
class CreateRentResponseScreen extends StatefulWidget {
  final RequestModel request;
  
  const CreateRentResponseScreen({
    super.key,
    required this.request,
  });

  @override
  State<CreateRentResponseScreen> createState() => _CreateRentResponseScreenState();
}

class _CreateRentResponseScreenState extends State<CreateRentResponseScreen> {
  ...
  Future<void> _selectAvailabilityDate({required bool isFromDate}) async {
    final DateTime? picked = await showDatePicker(
      ...
      initialDate: isFromDate 
        ? (DateTime.now())
        : (_availableFrom ?? DateTime.now()).add(const Duration(days: 1)),
    ...
    try {
      ...
      if (mounted) {
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
            content: Text('Rental offer submitted successfully!'),
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        title: const Text('Offer Rental Item'),
