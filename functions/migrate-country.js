...
async function addCountryToRequests() {
  ...
  return { updated: updatedCount, total: requestsSnapshot.docs.length };
}
...
async function addCountryToResponses() {
  ...
  return { updated: updatedCount, total: responsesSnapshot.docs.length };
}
...
async function addCountryToPriceListings() {
  ...
  return { updated: updatedCount, total: priceListingsSnapshot.docs.length };
}

// Main migration function as HTTP callable function
exports.migrateCountrySupport = functions.https.onCall(async (data, context) => {
  try {
    console.log('🌍 Starting country support migration for all collections...\n');

    const results = {
      requests: await addCountryToRequests(),
      responses: await addCountryToResponses(),
      priceListings: await addCountryToPriceListings()
    };
    ...
    return {
      success: true,
      results: results,
      message: 'Country support migration completed successfully!'
    };
