...
describe('Phone OTP Flow', () => {
  ...
  test('Send phone OTP stores record', async () => {
    ...
    const row = await db.queryOne('SELECT otp FROM phone_otp_verifications WHERE phone = $1', [testPhone]);
  ...
  test('Invalid phone OTP increments attempts', async () => {
    const before = await db.queryOne('SELECT attempts FROM phone_otp_verifications WHERE phone = $1', [testPhone]);
    const res = await request(app)
      .post('/api/auth/verify-phone-otp')
      .send({ phone: testPhone, otp: '000000' });
    expect(res.status).toBe(400);
    const after = await db.queryOne('SELECT attempts FROM phone_otp_verifications WHERE phone = $1', [testPhone]);
