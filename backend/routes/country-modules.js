...
router.get('/:countryCode', async (req, res) => {
  try {
    ...
    if (!row) {
      ...
      const modules = Object.fromEntries(DEFAULT_MODULE_KEYS.map(k=>[k,true]));
      return res.json({
        success: true,
        data: {
    ...
    // Merge defaults with stored config (stored values override default=true)
    const merged = { ...Object.fromEntries(DEFAULT_MODULE_KEYS.map(k=>[k,true])), ...(row.modules||{}) };
    row.modules = merged;
...
// POST /api/country-modules/:countryCode/auto-sync
// Seeds/updates categories, subcategories, and country business types for the given country
router.post('/:countryCode/auto-sync', auth.authMiddleware(), auth.roleMiddleware(['super_admin']), async (req, res) => {
  try {
    const { countryCode } = req.params;
    const seedPath = req.body?.seedPath; // optional custom seed
    const { reseed } = require('../scripts/reseed_categories_v2');

    // Load seed data file
    const fs = require('fs');
    const path = require('path');
    const defaultSeedPath = path.join(__dirname, '..', 'seed', 'categories.v2.json');
    const useSeedPath = seedPath && fs.existsSync(seedPath) ? seedPath : defaultSeedPath;
    const seed = JSON.parse(fs.readFileSync(useSeedPath,'utf8'));

    // Run reseed for categories/subcategories and country activation
    const result = await reseed({ seed, countryCode: countryCode.toUpperCase(), dryRun: false, purge: false });

    // Seed country_business_types using adminPermissions auto-activation helper
    const { autoActivateCountryData } = require('../services/adminPermissions');
    // Fake minimal country data (pull country name from countries table if available)
    const ctry = await db.queryOne('SELECT name FROM countries WHERE code=$1', [countryCode.toUpperCase()]);
    const countryName = ctry?.name || countryCode.toUpperCase();
    await autoActivateCountryData(countryCode.toUpperCase(), countryName, req.user.id);

    res.json({ success:true, message:'Auto sync completed', details: { reseed: result } });
  } catch (e) {
    console.error('Auto-sync error', e);
    res.status(500).json({ success:false, message:'Auto sync failed', error: e.message });
  }
});
